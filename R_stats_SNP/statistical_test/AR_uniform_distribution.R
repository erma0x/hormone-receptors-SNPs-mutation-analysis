...
"
    ...
    del gene, mi affido alla posizione massima e minima degli SNPs trovati in quel gene.
       ...
       QUERY al Database numero 26  (Solo per Androgene Receptor)
  
    query_26_AR <-paste('select SNPs.pos 
...
data <- read.csv('Tirocinio/Data/dati_26_AR.csv',header = TRUE) # read data
hist.output = hist(data, breaks = 1000) # divide into bins
real=hist.output$counts # take counts 
theor<-runif( length(real), min(real), max(real))
